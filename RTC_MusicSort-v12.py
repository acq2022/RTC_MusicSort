...
class Track:
    ...
    def __init__(self, root_path, title):
        ...
        # Sécurise problème de parsing lié à un mauvais encodage de tag
        self.securise_tag_of(tags, 'tracknumber')
        self.securise_tag_of(tags, 'tracktitle')
        self.securise_tag_of(tags, 'year')
        self.securise_tag_of(tags, 'album')
        self.securise_tag_of(tags, 'artist')
    ...
    def securise_tag_of(self, tags, tag_name):
        try: 
            tags[tag_name]
        except :
...
def copy_file_and_rename(source, destination, new_name, is_moving):
    ...
    if not os.path.exists(destination):
        ...
        write_in_report_file(destination + ' folder created\n')
...
def sort_tracks_of_folder(source_path, final_path, is_moving):
    ...
    for e in os.listdir(source_path):
        complete_source_path = os.path.join(source_path, str(e))
        
        # Dossier ?
        if(os.path.isdir(complete_source_path)):
                
            # Le dossier devient le nouvel emplacement à trier
            sort_tracks_of_folder(complete_source_path, final_path, is_moving)
            ...
            if(isTrack(e)):
                ...
                try:
                    ...
                    write_in_report_file('problem occurs with file ' + complete_source_path + ' : ' + str(er) + '\n')
            
            else:
                folder_destination = os.path.join(final_path, 'NOT PROCESSED')
                
                # Déplacer les fichiers
                copy_file_and_rename(complete_source_path, folder_destination, str(e), is_moving)
                
                write_in_report_file(complete_source_path + ' : file not processed')
...
def sort_button_activated():

    create_report_file(destination_path.get())
    show_animation_loader()
...
def show_success_window():
    ...
    write_in_report_file('FINISH WITH SUCCESS :)\n')
    close_report_file()
...
def create_report_file(dir_path):
    global text_file
    file_full_path = os.path.join(dir_path, 'report.txt')
    text_file = open(file_full_path, 'a', encoding='utf-8')
    print('report.txt created / opened')

    action = 'copy'
    if(action_type.get()):
        action = 'move'

    write_in_report_file('ROOT PATH : ' + root_path.get())
    write_in_report_file('DESTINATION PATH : ' + destination_path.get())
    write_in_report_file('action : ' + action + '\n')


def write_in_report_file(msg):
    global text_file
    text_file.write(msg + '\n')
    print(msg)
...
def close_report_file():
    global text_file
    text_file.close()
    print('report.txt closed')


def start_report_file():
    dir_path = destination_path.get()
    file_full_path = os.path.join(dir_path, 'report.txt')
    try:
...
button_sort = tkinter.Button(app, width = 9, text = 'Trier', state='disable', command = sort_button_activated)
